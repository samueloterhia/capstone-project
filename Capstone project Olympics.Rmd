...
athletereg %>% 
  group_by(Age) %>% 
  summarise(total = n()) %>% 
  ggplot(aes(x = Age)) + geom_histogram(aes(x= Age, y = total))
```
...
athletereg %>%
  ...
  geom_text(aes( x=region, y= Medal, label = region),vjust ="inward")
...
athletereg %>%
  ...
  group_by(Year) %>% 
  summarise(total = n()) %>% 
 ggplot(aes(x=Year, y= Sex)) + geom_col (aes(y=Sex), color = "red", size = 0.7)
